...
bool Matrices::Update(const float y)
{
    ...
    this->y[contador] = y;
    if (!(contador == (size_x-1))) return true;
    else return false;
}
...
void Matrices::Calcular()
{
    ...
    Matriz[2][2] = sum_x4;
    Matriz[2][3] = sum_yx2;
    //Habilitar en caso de querer confirmar el resultado calculado.
    /*
    for (int i = 0; i < 3; i++)
    {
        for (int j = 0; j < 4; j++)
...
void Matrices::get_Matriz()
{
    for (int i = 0; i < 5; i++)
    {
        for (int j = 0; j < 2; j++)
        {
            if(j==0) std::cout << x[i] << std::endl;
            else if(j==1) std::cout << y[i] << std::endl;
        }
        std::cout << std::endl;
    }
